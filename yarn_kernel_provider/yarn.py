...
from remote_kernel_provider.lifecycle import RemoteLifecycleManager
...
class YarnLifecycleManager(RemoteLifecycleManager):
    ...
    def __init__(self, kernel_manager, lifecycle_config):
        super(YarnLifecycleManager, self).__init__(kernel_manager, lifecycle_config)
...
# TODO - fix wait time - should just add member to k-m.
    ...
    def launch_process(self, kernel_cmd, **kwargs):
        ...
        super(YarnLifecycleManager, self).launch_process(kernel_cmd, **kwargs)
    ...
    def poll(self):
        ...
        if self._get_application_id():
            ...
            if state in YarnLifecycleManager.initial_states:
                result = None

        # The following produces too much output (every 3 seconds by default), so commented-out at this time.
        # self.log.debug("YarnLifecycleManager.poll, application ID: {}, kernel ID: {}, state: {}".
    ...
    def send_signal(self, signum):
        ...
        :return:
        """
        self.log.debug("YarnLifecycleManager.send_signal {}".format(signum))
        if signum == 0:
            return self.poll()
            ...
            return super(YarnLifecycleManager, self).send_signal(signum)
    ...
    def kill(self):
        ...
        if self._get_application_id():
            ...
            self.log.debug(
                "YarnLifecycleManager.kill: application ID: {}, confirming application state is not RUNNING"
                    .format(self.application_id, resp))

            i = 1
            state = self._query_app_state_by_id(self.application_id)
            while state not in YarnLifecycleManager.final_states and i <= max_poll_attempts:
                time.sleep(poll_interval)
                state = self._query_app_state_by_id(self.application_id)
                i = i + 1

            if state in YarnLifecycleManager.final_states:
                result = None

        if result is False:
            super(YarnLifecycleManager, self).kill()

        self.log.debug("YarnLifecycleManager.kill, application ID: {}, kernel ID: {}, state: {}"
                       .format(self.application_id, self.kernel_id, state))
        return result
    ...
    def cleanup(self):
        ...
        # a local_proc.
        if self.local_proc:
            self.log.debug("YarnLifecycleManager.cleanup: Clearing possible defunct process, pid={}...".
                           format(self.local_proc.pid))
            if super(YarnLifecycleManager, self).poll():
                super(YarnLifecycleManager, self).kill()
            super(YarnLifecycleManager, self).wait()
        ...
        super(YarnLifecycleManager, self).cleanup()
    ...
    def confirm_remote_startup(self):
        ...
        """
        self.start_time = RemoteLifecycleManager.get_current_time()
        i = 0
        ...
        while not ready_to_connect:
            ...
            if self._get_application_id(True):
                ...
                if app_state in YarnLifecycleManager.final_states:
    ...
    def handle_timeout(self):
        ...
        time.sleep(poll_interval)
        time_interval = RemoteLifecycleManager.get_time_diff(self.start_time, RemoteLifecycleManager.get_current_time())
    ...
    def _get_application_id(self, ignore_final_states=False):
        ...
        if not self.application_id:
            ...
            if type(app) is dict and ignore_final_states:
                state_condition = app.get('state') not in YarnLifecycleManager.final_states
    ...
    def get_lifecycle_info(self):
        ...
        lifecycle_info = super(YarnLifecycleManager, self).get_lifecycle_info()
    ...
    def load_lifecycle_info(self, lifecycle_info):
        ...
        super(YarnLifecycleManager, self).load_lifecycle_info(lifecycle_info)
